...
def calibration(calibration_time,sps,adc, ADS, freq_min=8,freq_max=12,print_time=False):
    """
    ...
    chan0 = AnalogIn(adc, ADS.P2, ADS.P3)
    ...
    for i in range(nsamples): #Collects data every sinterval
        ...
        print("chan0.voltage analysis_tools.py")
        time_series[i] = chan0.voltage # Read voltage from differential inputs
...
def get_cutoff(num_samples, calibration_time, sps, adc, ADS):
    """
    ...
    for i in range(num_samples):
        ...
        relaxed_rms[i] = calibration(calibration_time,sps,adc, ADS)
        input("Press <Enter> to record concentrated state")
        concentrated_rms[i] = calibration(calibration_time,sps,adc, ADS)
